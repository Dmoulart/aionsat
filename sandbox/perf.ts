...
const shapes = generateShapes(300)
...
(function loop() {
  ...
  for (let i = 0; i < shapes.length; i++) {
    shapes[i].pos = Math.random() > 0.5 ? shapes[i].pos.add(vec(Math.random() * 2, Math.random() * 2)) : shapes[i].pos.add(vec(-Math.random() * 2, -Math.random() * 2))
